...
public class PcAuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		/**
		 * assert 断言
		 * 1: assert <boolean表达式> 如果为true,则程序继续执行,如果为false
		 * 则程序抛出AssertionError,并终止执行
		 *
		 * 2: assert <boolean表达式>: <错误信息表达式>
		 * 如果为true,则程序继续执行
		 * false,则程序抛出java.lang.AssertionError,并输出<错误信息表达式>
		 */
		assert tokens.length == 2 : "assert断言不成立,检查token值";
