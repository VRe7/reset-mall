...
//			uacGroupUserService.save(groupUser);
			...
			// 2.绑定组织信息
			UacGroupUser uacGroupUser = null;//uacGroupUserService.queryByUserId(user.getId());
	...
	public void bindUserRoles(BindUserRolesDto bindUserRolesDto, LoginAuthDto authResDto) {
		...
		List<UacRoleUser> userRoles = null;//uacRoleUserService.listByUserId(operUserId);
		...
		for (Long roleId : roleIdList) {
			UacRole uacRole = null;//uacRoleService.getRoleById(roleId);
			if (uacRole == null) {
	...
	public List<UserMenuDto> queryUserMenuDtoData(LoginAuthDto authResDto) {
		...
		menuList = null;// uacMenuService.getMenuVoList(userId, 1L);
	...
	public UserBindRoleVo getUserBindRoleDto(Long userId) {
		...
		// 该角色已经绑定的用户
		List<UacRoleUser> setAlreadyBindRoleSet = null;//uacRoleUserService.listByUserId(userId);
	...
	public void activeUser(String activeUserToken) {
		...
		String email = null;//redisService.getKey(activeUserKey);
	...
	private void validateEmailResetPwd(ForgetResetPasswordDto forgetResetPasswordDto) {
		...
		String forgetKey = null;//redisService.getKey(key);
	...
	private void checkUserMenuList(List<Long> menuIdList, LoginAuthDto authResDto, List<UacUserMenu> uacUserMenuList) {
		...
		List<MenuVo> currentUserMenuVoList = null;//uacMenuService.findAllMenuListByAuthResDto(authResDto);
